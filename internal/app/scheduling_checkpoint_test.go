...
func TestRuntimeCheckpointRestoresOnlyMatchingSchedulingIdentities(t *testing.T) {
	...
	if err := NewFileRuntimeStateCheckpoint(dir, original, nil, nil, nil).Save(context.Background()); err != nil {
		t.Fatal(err)
	}
	loaded := makeRegistry(map[uint]uint64{1: 1, 2: 2, 3: 1})
	if err := NewFileRuntimeStateCheckpoint(dir, loaded, nil, nil, nil).Restore(context.Background()); err != nil {
		t.Fatal(err)
	}
