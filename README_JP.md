...
- 2.0 は既定で**単一アプリケーションインスタンス**として動作します。`REDIS_ADDRS` を設定すると実験的なクラスターモードが有効になります（PostgreSQL と明示的な `AUTH_KEY`/`ENCRYPTION_KEY` が必要）。現在のバージョンでは設定変更と AccessKey の RPM 制限・コスト上限をインスタンス間で共有し、認証情報のヘルスと Responses の継続はインスタンスローカルのままで、サブスクリプションチャネルはまだ複数インスタンスに対応していません。Redis が利用できない場合、RPM 制限またはコスト上限を設定した AccessKey のリクエストはアップストリームへ転送されず、503 `cluster_state_unavailable` を返します。Redis には `maxmemory-policy noeviction` を設定し、AOF 永続化を有効にすることを推奨します。Redis のデータが失われた場合、使用済みコストは最後のデータベースチェックポイント（約 1 秒前）に戻ります。各インスタンスの時刻は NTP で同期してください。
