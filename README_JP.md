...
- 2.0 は既定で**単一アプリケーションインスタンス**として動作します。`REDIS_ADDRS` を設定すると実験的なクラスターモードが有効になります（PostgreSQL と明示的な `AUTH_KEY`/`ENCRYPTION_KEY` が必要）。現在のバージョンでは設定変更のみをインスタンス間で共有し、AccessKey のレート制限とクォータ、認証情報のヘルス、Responses の継続はインスタンスローカルのままで、サブスクリプションチャネルはまだ複数インスタンスに対応していません。
