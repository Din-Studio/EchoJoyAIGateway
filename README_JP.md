...
| `REDIS_ADDRS` | 空、クラスターモード無効 | カンマ区切りの Redis アドレス。設定すると実験的なクラスターモードが有効になります。PostgreSQL の `DATABASE_DSN` と明示的な `AUTH_KEY`、`ENCRYPTION_KEY` が必要で、起動時に Redis へ到達できる必要があります。Redis Cluster では全ノードのアドレスを列挙してください。 |
| `REDIS_PASSWORD` | 空 | クラスターモードで使用する Redis のパスワード。 |
| `REDIS_TLS` | `false` | クラスターモードで Redis に TLS で接続するかどうか。 |
| `REDIS_KEY_PREFIX` | `gl` | クラスターモードのすべての Redis キーとチャネルの接頭辞。空白を含めたり `:` で終わらせたりすることはできません。 |
| `INSTANCE_ID` | `<ホスト名>-<ランダム>` | クラスターイベントにおけるこのプロセスの識別子。インスタンスごとに一意である必要があります。 |
...
- 2.0 は既定で**単一アプリケーションインスタンス**として動作します。`REDIS_ADDRS` を設定すると実験的なクラスターモードが有効になります（PostgreSQL と明示的な `AUTH_KEY`/`ENCRYPTION_KEY` が必要）。現在のバージョンでは設定変更、AccessKey の RPM 制限・コスト上限、認証情報のヘルス（クールダウン、ブラックリスト、失敗回数、モデルクールダウン）、サブスクリプションの認可状態をインスタンス間で共有し、同じサブスクリプションアカウントのリフレッシュはクラスター内で同時に 1 インスタンスだけが実行します。Responses の継続はインスタンスローカルのままです。認証情報のヘルス統計ページはそのインスタンスのビューで、検証プローブは各インスタンスで実行されます。サブスクリプションアカウントをリフレッシュ中のインスタンスがクラッシュした場合、そのアカウントは約 60 秒以内（リース TTL とポーリング 1 周期）に `outcome_unknown` になり、手動での復旧が必要です。Redis に書き込めなかったヘルスの変更はまずそのインスタンスで適用され、Redis の復旧後は Redis の状態が優先されます。Redis が利用できない場合、RPM 制限またはコスト上限を設定した AccessKey のリクエストはアップストリームへ転送されず、503 `cluster_state_unavailable` を返します。Redis には `maxmemory-policy noeviction` を設定し、AOF 永続化を有効にすることを推奨します。Redis のデータが失われた場合、使用済みコストは最後のデータベースチェックポイント（約 1 秒前）に戻ります。各インスタンスの時刻は NTP で同期してください。
